...
contract MyTokenTest is Context, IERC20, Ownable {
    ...
    // Set to 1%.
    uint256 private doubleTaxPercent = 10; // Less fields to edit
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
        require(taxFee <= 8, "Prevents owner from abusing fees."); // Prevents owner from abusing fees.
        _taxFee = taxFee;
    }
    
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
        require(liquidityFee <= 10, "Prevents owner from abusing fees."); // Prevents owner from abusing fees.
        _liquidityFee = liquidityFee;
    }

    function setMarketingFeePercent(uint256 marketingFee) external onlyOwner() {
        require(marketingFee <= 10, "Prevents owner from abusing fees."); // Prevents owner from abusing fees.
    ...
    function setMaxTxPercent(uint256 percent, uint256 divisor) external onlyOwner() {
        require(divisor <= 10000, "Cannot set lower than 0.01%"); // Cannot set lower than 0.01%
    ...
    function setMaxWalletSize(uint256 percent, uint256 divisor) external onlyOwner() {
        require(divisor <= 1000, "Cannot set lower than 0.01%"); // Cannot set lower than 0.1%
